...
local johnTrackIndex = getTrackIndexByName("JOHN")
local partGuitarTrackIndex = getTrackIndexByName("PART GUITAR")

if not johnTrackIndex or not partGuitarTrackIndex then
    log("Error: Couldn't find the tracks.")
    return
end

local johnTrack = reaper.GetTrack(0, johnTrackIndex)
local partGuitarTrack = reaper.GetTrack(0, partGuitarTrackIndex)

local johnTake = reaper.GetTake(reaper.GetTrackMediaItem(johnTrack, 0), 0)
local partGuitarTake = reaper.GetTake(reaper.GetTrackMediaItem(partGuitarTrack, 0), 0)

if not johnTake or not partGuitarTake then
    log("Error: Couldn't find the takes for the tracks.")
    return
end

local _, noteCount, _, _ = reaper.MIDI_CountEvts(johnTake)
log("Number of events in JOHN: " .. noteCount)

local lastNoteEndPPQ = 0

-- First pass to identify gaps between MIDI notes 60 to 69
for i = 0, noteCount - 1 do
    local _, _, _, startppq, endppq, _, pitch, _ = reaper.MIDI_GetNote(johnTake, i)
...
for i = noteCount - 1, 0, -1 do
    local _, _, _, startppq, endppq, _, pitch, velocity = reaper.MIDI_GetNote(johnTake, i)
    ...
    if textEvent then
        ...
        reaper.MIDI_DeleteNote(johnTake, i)
...
reaper.DeleteTrack(johnTrack)
log("Deleted JOHN track")

reaper.Undo_EndBlock("Convert MIDI notes from JOHN to Text Events in PART GUITAR and copy MIDI notes 41-57", -1)
